...
class IttCrossModel(IttPreTrainedModel):
    ...
    def forward(self, input_part0, input_part1, input_part2, graph_data, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, use_cross_attention_mask=True, ablation=None):
        ...
        if ablation == 'PIH':
            part0_emb = torch.zeros_like(part0_emb)
            part1_emb = torch.zeros_like(part1_emb)
            part2_emb = torch.zeros_like(part2_emb)
        if ablation == 'Atomic':
        ...
        if attention_mask is not None:
            if ablation == 'Atomic':
                graph_token_mask = torch.zeros(
                    (attention_mask.size(0), self.config.max_atom_number),
            ...
            if ablation == 'PIH':
                part0_mask = torch.zeros(
                    (attention_mask.size(0), self.config.n0),
                    device=attention_mask.device,
                    dtype=attention_mask.dtype
                )
                part1_mask = torch.zeros(
                    (attention_mask.size(0), self.config.n1),
                    device=attention_mask.device,
                    dtype=attention_mask.dtype
                )
                part2_mask = torch.zeros(
                    (attention_mask.size(0), self.config.n2),
                    device=attention_mask.device,
                    dtype=attention_mask.dtype
                )
            else:
                part0_mask = attention_mask[:, :self.config.n0]
                part1_mask = attention_mask[:, self.config.n0:self.config.n0+self.config.n1]
...
class IttCrossSequenceClassifier(IttPreTrainedModel):
    ...
    def forward(self, input_part0, input_part1, input_part2, graph_data,
                attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, labels=None, use_cross_attention_mask=False, ablation=None):
        ...
        if self.classifier_head_method == 'cls':
            ...
            if ablation == 'PIH':
                part0_pooled = torch.zeros_like(torch.mean(graph_emb, dim=1))
                part1_pooled = torch.zeros_like(torch.mean(graph_emb, dim=1))
            else:
                part0_pooled = torch.mean(part0_emb, dim=1)
                part1_pooled = torch.mean(part1_emb, dim=1)

            if ablation == 'Atomic':
                graph_pooled = torch.zeros_like(part0_pooled)
            else:
